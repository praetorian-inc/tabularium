...
type Port struct {
	...
	Protocol   string            `neo4j:"protocol" json:"protocol" desc:"The protocol of this port." example:"tcp" capmodel:"Port"`
	Port       int               `neo4j:"port" json:"port" desc:"The port number of this port." example:"80" capmodel:"Port"`
	Service    string            `neo4j:"service" json:"service" desc:"The name of the service identified on this port." example:"https" capmodel:"Port"`
	...
	Parent     GraphModelWrapper `neo4j:"-" json:"parent" desc:"Port parent asset." capmodel:"Port=parent(Asset)"`
	Tags
}
...
func (p *Port) Valid() bool {
	return p.Key != "" && p.Port > 0 && p.Port <= 65535
}
	...
	return []registry.Hook{
		{
			Call: func() error {
				...
				p.Key = fmt.Sprintf("#port#%s#%d%s", p.Protocol, p.Port, p.Parent.Model.GetKey())
