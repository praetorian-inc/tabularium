...
func TestAttribute_Preseed(t *testing.T) {
	tests := []struct {
		...
		// {
		// 	name: "basic preseed",
		// 	attr: NewAttribute("preseed", "#preseed#whois+company#company#Chariot Systems", &Seed{Key: "#seed#domain#gladiator.systems"}),
		...
		// {
		// 	name: "preseed source",
		// 	attr: NewAttribute("source", "#preseed#whois+company#company#Chariot Systems", &Seed{Key: "#seed#domain#gladiator.systems"}),
		// 	want: NewPreseed("whois+company", "company", "Chariot Systems"),
		// },
	}
	...
	for _, test := range tests {
		...
		if !reflect.DeepEqual(actual, test.want) {
			t.Errorf("Attribute.Preseed() = %v, want %v", actual, test.want)
		}
	}
}
