...
func TestRepository_Visit_MergesLastScannedCommits(t *testing.T) {
	repo := NewRepository("https://github.com/org/repo")
	repo.LastScannedCommits = map[string]string{
		"constantine": "aaa",
		"trufflehog":  "bbb",
	}

	other := NewRepository("https://github.com/org/repo")
	other.LastScannedCommits = map[string]string{
		"constantine": "ccc",
	}

	repo.Visit(&other)

	// "constantine" updated to other's value
	assert.Equal(t, "ccc", repo.LastScannedCommits["constantine"])
	// "trufflehog" preserved — other didn't touch it
	assert.Equal(t, "bbb", repo.LastScannedCommits["trufflehog"])
}

func TestRepository_Visit_NilOtherLastScannedCommits(t *testing.T) {
	repo := NewRepository("https://github.com/org/repo")
	repo.LastScannedCommits = map[string]string{
		"constantine": "aaa",
	}
	...
	other := NewRepository("https://github.com/org/repo")
	// other.LastScannedCommits is nil

	repo.Visit(&other)

	// Existing map untouched
	assert.Equal(t, map[string]string{"constantine": "aaa"}, repo.LastScannedCommits)
}

func TestRepository_Visit_EmptyOtherLastScannedCommits(t *testing.T) {
	repo := NewRepository("https://github.com/org/repo")
	repo.LastScannedCommits = map[string]string{
		"constantine": "aaa",
	}
	...
	// Empty map treated same as nil — existing preserved
	assert.Equal(t, map[string]string{"constantine": "aaa"}, repo.LastScannedCommits)
}

func TestRepository_Visit_NilReceiverLastScannedCommits(t *testing.T) {
	repo := NewRepository("https://github.com/org/repo")
	// repo.LastScannedCommits is nil
	...
	// Map created from scratch with other's values
	assert.Equal(t, map[string]string{"constantine": "aaa"}, repo.LastScannedCommits)
}

func TestRepository_Visit_BothNilLastScannedCommits(t *testing.T) {
	repo := NewRepository("https://github.com/org/repo")
	other := NewRepository("https://github.com/org/repo")
	...
	// Both nil — stays nil
	assert.Nil(t, repo.LastScannedCommits)
}
