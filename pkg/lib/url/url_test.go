...
func TestGetSiteName(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:     "https://very-long-hostname-that-exceeds-limits.example.com/path",
			maxLength: 20,
			expected:  "very-long-hostname-t",
			wantErr:   false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := GetSiteName(tt.input, tt.maxLength)
			if (err != nil) != tt.wantErr {
				t.Errorf("GetSiteName() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if result != tt.expected {
				t.Errorf("GetSiteName() = %v, expected %v", result, tt.expected)
			}
		})
	}
}
