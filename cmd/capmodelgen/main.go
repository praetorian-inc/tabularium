...
// modelOutput holds the collected information for a single capability model type.
...
func main() {
	...
	buf.WriteString("// Code generated by capmodelgen. DO NOT EDIT.\n\n")
	buf.WriteString("package capmodel\n\n")
	...
	// Generate each capability model type
	for _, m := range models {
		typeName := m.pascalName

		fmt.Fprintf(&buf, "// %s is a capability model for %s.\n", typeName, m.pascalName)
		fmt.Fprintf(&buf, "type %s struct {\n", typeName)

		if m.parent != parentNone {
			fmt.Fprintf(&buf, "\tAsset Asset `json:\"-\"`\n")
		}
		...
		if m.parent != parentNone {
			...
			fmt.Fprintf(&buf, "func (s %s) GetParentAsset() (Asset, bool) { return s.Asset, %t }\n\n", typeName, m.parent == parentInject)
		}
	...
	fmt.Printf("Generated capability model types at %s\n", outputPath)
}
